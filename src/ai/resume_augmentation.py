...
def run_resume_campaign(start_qid=36, batch_size=50):
    ...
    # Filter questions to start from start_qid
    remaining_questions = [q for q in questions if int(q['id']) >= start_qid]
    
    print(f"--- RESUMING AUGMENTATION FROM QID {start_qid} ---")
    
    count = 2150 # Current count in CSV as per our analysis
    session_counter = 0
    batch_results = []
    ...
    for q in remaining_questions:
        ...
        for s in gps_students:
            try:
                print(f"🔄 Đang sinh GPS: {s['id']} | QID: {q['id']} (Tổng: {2100 + session_counter + 1}/2700)...")
            ...
            if len(batch_results) >= batch_size:
                ...
                batch_results = []
                print(f"💾 Đã lưu đợt dữ liệu (Tổng tích lũy: {2100 + session_counter})")

        # Group Non-GPS
        for s in nongps_students:
            try:
                print(f"🔄 Đang sinh Non-GPS: {s['id']} | QID: {q['id']} (Tổng: {2100 + session_counter + 1}/2700)...")
    ...
    print(f"\n🎉 RESUME COMPLETE. Added {session_counter} sessions.")
